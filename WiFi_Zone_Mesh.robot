...
No.40 Connect to Common SSID 2G controller
    [Tags]    2G
    Modify WPA Supplicant wpa3 Config    ${2G_SSID}    ${2G_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${2G_SSID_BSSID}    
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${2G_SSID_BSSID}

No.41 Connect to Common SSID 5G controller
    [Tags]    5G
    Modify WPA Supplicant wpa3 Config    ${5G_SSID}    ${5G_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${5G_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${5G_SSID_BSSID}

No.42 Connect to Common SSID 6G controller
    [Tags]    6G
    Modify WPA Supplicant wpa3 Config    ${6G_SSID}    ${6G_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${6G_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${6G_SSID_BSSID}

No.43 Connect to Common SSID 2G agent
    [Tags]    Agent    2G
    Modify WPA Supplicant wpa3 Config    ${2G_Agent_SSID}    ${2G_Agent_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${2G_Agent_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${2G_Agent_SSID_BSSID}

No.44 Connect to Common SSID 5G agent
    [Tags]    Agent    5G
    Modify WPA Supplicant wpa3 Config    ${5G_Agent_SSID}    ${5G_Agent_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${5G_Agent_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${5G_Agent_SSID_BSSID}

No.45 Connect to Common SSID 6G agent
    [Tags]    Agent    6G
    Modify WPA Supplicant wpa3 Config    ${6G_Agent_SSID}    ${6G_Agent_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${6G_Agent_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${6G_Agent_SSID_BSSID}

No.46 Connect to Controller 2G Guest Captive Portal
    [Tags]    Guest    Captive    2G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.47 Connect to Controller 5G Guest Captive Portal
    [Tags]    Guest    Captive    5G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.48 Connect to Controller 6G Guest Captive Portal
    [Tags]    Guest    Captive    6G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.49 Connect to Agent 2G Guest Captive Portal
    [Tags]    Guest    Captive    2G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.50 Connect to Agent 5G Guest Captive Portal
    [Tags]    Guest    Captive    5G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.51 Connect to Agent 6G Guest Captive Portal
    [Tags]    Guest    Captive    6G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.52 Change the Agent to Wireless connection
    [Tags]    Agent    Wireless    Connection
    Disable the switch port connected to the agent
    sleep    300
    Enable Client wired Network
...
No.52 Connect to Changed Guest SSID 2G Controller
    [Tags]    Guest    Changed    2G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.53 Connect to Changed Guest SSID 5G Controller
    [Tags]    Guest    Changed    5G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.54 Connect to Changed Guest SSID 6G Controller
    [Tags]    Guest    Changed    6G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.55 Connect to Changed Guest SSID 2G Agent
    [Tags]    Guest    Changed    2G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.56 Connect to Changed Guest SSID 5G Agent
    [Tags]    Guest    Changed    5G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.57 Connect to Changed Guest SSID 6G Agent
    [Tags]    Guest    Changed    6G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.58 Connect to Controller 2G Employee Fronthaul SSID
    [Tags]    Employee    2G    Controller
    Connect to Employee ssid and Check client can get ip    ${2G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.59 Connect to Controller 5G Employee Fronthaul SSID
    [Tags]    Employee    5G    Controller
    Connect to Employee ssid and Check client can get ip    ${5G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.60 Connect to Controller 6G Employee Fronthaul SSID
    [Tags]    Employee    6G    Controller
    Connect to Employee ssid and Check client can get ip    ${6G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.61 Connect to Agent 2G Employee Fronthaul SSID
    [Tags]    Employee    2G    Agent
    Connect to Employee ssid and Check client can get ip    ${2G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.62 Connect to Agent 5G Employee Fronthaul SSID
    [Tags]    Employee    5G    Agent
    Connect to Employee ssid and Check client can get ip    ${5G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.63 Connect to Agent 6G Employee Fronthaul SSID
    [Tags]    Employee    6G    Agent
    Connect to Employee ssid and Check client can get ip    ${6G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.64 Connect to Changed Employee SSID 2G Controller
    [Tags]    Employee    Changed    2G    Controller
    Connect to Employee ssid and Check client can get ip    ${2G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.65 Connect to Changed Employee SSID 5G Controller
    [Tags]    Employee    Changed    5G    Controller
    Connect to Employee ssid and Check client can get ip    ${5G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.66 Connect to Changed Employee SSID 6G Controller
    [Tags]    Employee    Changed    6G    Controller
    Connect to Employee ssid and Check client can get ip    ${6G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.67 Connect to Changed Employee SSID 2G Agent
    [Tags]    Employee    Changed    2G    Agent
    Connect to Employee ssid and Check client can get ip    ${2G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.68 Connect to Changed Employee SSID 5G Agent
    [Tags]    Employee    Changed    5G    Agent
    Connect to Employee ssid and Check client can get ip    ${5G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.69 Connect to Changed Employee SSID 6G Agent
    [Tags]    Employee    Changed    6G    Agent
    Connect to Employee ssid and Check client can get ip    ${6G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
Disable Guest Captive Portal
    [Tags]    Guest    CaptivePortal
    Check Guest Captive Portal status and Disable it

No.71 Connect to Controller 2G Guest Fronthaul SSID
...
No.72 Connect to Controller 5G Guest Fronthaul SSID
...
No.73 Connect to Controller 6G Guest Fronthaul SSID
...
No.74 Connect to Agent 2G Guest Fronthaul SSID
...
No.75 Connect to Agent 5G Guest Fronthaul SSID
...
No.76 Connect to Agent 6G Guest Fronthaul SSID
...
No.78 Connect to Common SSID 2G controller
    [Tags]    2G
    Modify WPA Supplicant wpa3 Config    ${2G_SSID}    ${2G_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${2G_SSID_BSSID}    
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${2G_SSID_BSSID}

No.79 Connect to Common SSID 5G controller
    [Tags]    5G
    Modify WPA Supplicant wpa3 Config    ${5G_SSID}    ${5G_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${5G_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${5G_SSID_BSSID}

No.80 Connect to Common SSID 6G controller
    [Tags]    6G
    Modify WPA Supplicant wpa3 Config    ${6G_SSID}    ${6G_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${6G_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${6G_SSID_BSSID}

No.81 Connect to Common SSID 2G agent
    [Tags]    Agent    2G
    Modify WPA Supplicant wpa3 Config    ${2G_Agent_SSID}    ${2G_Agent_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${2G_Agent_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${2G_Agent_SSID_BSSID}

No.82 Connect to Common SSID 5G agent
    [Tags]    Agent    5G
    Modify WPA Supplicant wpa3 Config    ${5G_Agent_SSID}    ${5G_Agent_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${5G_Agent_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${5G_Agent_SSID_BSSID}

No.83 Connect to Common SSID 6G agent
    [Tags]    Agent    6G
    Modify WPA Supplicant wpa3 Config    ${6G_Agent_SSID}    ${6G_Agent_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${6G_Agent_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${6G_Agent_SSID_BSSID}

No.84 Connect to Controller 2G Employee Fronthaul SSID
    [Tags]    Employee    2G    Controller
    Connect to Employee ssid and Check client can get ip    ${2G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.85 Connect to Controller 5G Employee Fronthaul SSID
    [Tags]    Employee    5G    Controller
    Connect to Employee ssid and Check client can get ip    ${5G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.86 Connect to Controller 6G Employee Fronthaul SSID
    [Tags]    Employee    6G    Controller
    Connect to Employee ssid and Check client can get ip    ${6G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.87 Connect to Agent 2G Employee Fronthaul SSID
    [Tags]    Employee    2G    Agent
    Connect to Employee ssid and Check client can get ip    ${2G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.88 Connect to Agent 5G Employee Fronthaul SSID
    [Tags]    Employee    5G    Agent
    Connect to Employee ssid and Check client can get ip    ${5G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.89 Connect to Agent 6G Employee Fronthaul SSID
    [Tags]    Employee    6G    Agent
    Connect to Employee ssid and Check client can get ip    ${6G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.90 Connect to Changed Employee SSID 2G Controller
    [Tags]    Employee    Changed    2G    Controller
    Connect to Employee ssid and Check client can get ip    ${2G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.91 Connect to Changed Employee SSID 5G Controller
    [Tags]    Employee    Changed    5G    Controller
    Connect to Employee ssid and Check client can get ip    ${5G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.92 Connect to Changed Employee SSID 6G Controller
    [Tags]    Employee    Changed    6G    Controller
    Connect to Employee ssid and Check client can get ip    ${6G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.93 Connect to Changed Employee SSID 2G Agent
    [Tags]    Employee    Changed    2G    Agent
    Connect to Employee ssid and Check client can get ip    ${2G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.94 Connect to Changed Employee SSID 5G Agent
    [Tags]    Employee    Changed    5G    Agent
    Connect to Employee ssid and Check client can get ip    ${5G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.95 Connect to Changed Employee SSID 6G Agent
    [Tags]    Employee    Changed    6G    Agent
    Connect to Employee ssid and Check client can get ip    ${6G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.96 Change the Agent to Wireless connection
    [Tags]    Agent    Wireless    Connection
    Disable the switch port connected to the agent
    sleep    300
    Enable Client wired Network
...
No.97 Connect to Guest Fronthaul SSID 2G controller
    [Tags]    Guest    2G    Controller
    Connect to Guest ssid and Check client can get ip    ${2G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.98 Connect to Guest Fronthaul SSID 5G controller
    [Tags]    Guest    5G    Controller
    Connect to Guest ssid and Check client can get ip    ${5G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.99 Connect to Guest Fronthaul SSID 6G controller
    [Tags]    Guest    6G    Controller
    Connect to Guest ssid and Check client can get ip    ${6G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.100 Connect to Guest Fronthaul SSID 2G agent
    [Tags]    Guest    2G    Agent
    Connect to Guest ssid and Check client can get ip    ${2G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.101 Connect to Guest Fronthaul SSID 5G agent
    [Tags]    Guest    5G    Agent
    Connect to Guest ssid and Check client can get ip    ${5G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.102 Connect to Guest Fronthaul SSID 6G agent
    [Tags]    Guest    6G    Agent
    Connect to Guest ssid and Check client can get ip    ${6G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.103 Connect to Controller 2G Guest Captive Portal
    [Tags]    Guest    Captive    2G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.104 Connect to Controller 5G Guest Captive Portal
    [Tags]    Guest    Captive    5G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.105 Connect to Controller 6G Guest Captive Portal
    [Tags]    Guest    Captive    6G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.106 Connect to Agent 2G Guest Captive Portal
    [Tags]    Guest    Captive    2G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.107 Connect to Agent 5G Guest Captive Portal
    [Tags]    Guest    Captive    5G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.108 Connect to Agent 6G Guest Captive Portal
    [Tags]    Guest    Captive    6G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.109 Connect to Changed Guest SSID 2G Controller
    [Tags]    Guest    Changed    2G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.110 Connect to Changed Guest SSID 5G Controller
    [Tags]    Guest    Changed    5G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.111 Connect to Changed Guest SSID 6G Controller
    [Tags]    Guest    Changed    6G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.112 Connect to Changed Guest SSID 2G Agent
    [Tags]    Guest    Changed    2G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.113 Connect to Changed Guest SSID 5G Agent
    [Tags]    Guest    Changed    5G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.114 Connect to Changed Guest SSID 6G Agent
    [Tags]    Guest    Changed    6G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
Enable Guest , Guest Captive Portal and Employee
    [Tags]    Guest    Mesh
    Enable Guest , Guest Captive Portal and Employee

No.115 Check Mesh Agent is online
    [Tags]    Agent
    Enable the switch port connected to the agent
    sleep    300
    Get agent ip
...
No.116 Connect to Common SSID 2G controller
    [Tags]    2G
    Modify WPA Supplicant wpa3 Config    ${2G_SSID}    ${2G_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${2G_SSID_BSSID}    
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${2G_SSID_BSSID}

No.117 Connect to Common SSID 5G controller
    [Tags]    5G
    Modify WPA Supplicant wpa3 Config    ${5G_SSID}    ${5G_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${5G_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${5G_SSID_BSSID}

No.118 Connect to Common SSID 6G controller
    [Tags]    6G
    Modify WPA Supplicant wpa3 Config    ${6G_SSID}    ${6G_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${6G_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${6G_SSID_BSSID}

No.119 Connect to Common SSID 2G agent
    [Tags]    Agent    2G
    Modify WPA Supplicant wpa3 Config    ${2G_Agent_SSID}    ${2G_Agent_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${2G_Agent_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${2G_Agent_SSID_BSSID}

No.120 Connect to Common SSID 5G agent
    [Tags]    Agent    5G
    Modify WPA Supplicant wpa3 Config    ${5G_Agent_SSID}    ${5G_Agent_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${5G_Agent_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${5G_Agent_SSID_BSSID}

No.121 Connect to Common SSID 6G agent
    [Tags]    Agent    6G
    Modify WPA Supplicant wpa3 Config    ${6G_Agent_SSID}    ${6G_Agent_SSID_BSSID}    ${Default_PSK}
    Connect to ssid and ping controller and agent    ${6G_Agent_SSID_BSSID} 
    [Teardown]    Run Keyword If    '${TEST STATUS}' == 'FAIL'    Check SSID can be scan    ${6G_Agent_SSID_BSSID}

No.122 Connect to Controller 2G Guest Captive Portal
    [Tags]    Guest    Captive    2G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.123 Connect to Controller 5G Guest Captive Portal
    [Tags]    Guest    Captive    5G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.124 Connect to Controller 6G Guest Captive Portal
    [Tags]    Guest    Captive    6G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.125 Connect to Agent 2G Guest Captive Portal
    [Tags]    Guest    Captive    2G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.126 Connect to Agent 5G Guest Captive Portal
    [Tags]    Guest    Captive    5G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.127 Connect to Agent 6G Guest Captive Portal
    [Tags]    Guest    Captive    6G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.128 Connect to Changed Guest SSID 2G Controller
    [Tags]    Guest    Changed    2G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.129 Connect to Changed Guest SSID 5G Controller
    [Tags]    Guest    Changed    5G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.130 Connect to Changed Guest SSID 6G Controller
    [Tags]    Guest    Changed    6G    Controller
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Controller_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.131 Connect to Changed Guest SSID 2G Agent
    [Tags]    Guest    Changed    2G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${2G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth
...
No.132 Connect to Changed Guest SSID 5G Agent
    [Tags]    Guest    Changed    5G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${5G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 5G
...
No.133 Connect to Changed Guest SSID 6G Agent
    [Tags]    Guest    Changed    6G    Agent
    Connect to Guest ssid and Check client can get ip > portal    ${6G_Agent_Guest_SSID}
    Verify Client cannot open GUI
    Send Guest Captive Portal Auth 6G
...
No.134 Connect to Controller 2G Employee Fronthaul SSID
    [Tags]    Employee    2G    Controller
    Connect to Employee ssid and Check client can get ip    ${2G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.135 Connect to Controller 5G Employee Fronthaul SSID
    [Tags]    Employee    5G    Controller
    Connect to Employee ssid and Check client can get ip    ${5G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.136 Connect to Controller 6G Employee Fronthaul SSID
    [Tags]    Employee    6G    Controller
    Connect to Employee ssid and Check client can get ip    ${6G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.137 Connect to Agent 2G Employee Fronthaul SSID
    [Tags]    Employee    2G    Agent
    Connect to Employee ssid and Check client can get ip    ${2G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.138 Connect to Agent 5G Employee Fronthaul SSID
    [Tags]    Employee    5G    Agent
    Connect to Employee ssid and Check client can get ip    ${5G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.139 Connect to Agent 6G Employee Fronthaul SSID
    [Tags]    Employee    6G    Agent
    Connect to Employee ssid and Check client can get ip    ${6G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.140 Connect to Changed Employee SSID 2G Controller
    [Tags]    Employee    Changed    2G    Controller
    Connect to Employee ssid and Check client can get ip    ${2G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.141 Connect to Changed Employee SSID 5G Controller
    [Tags]    Employee    Changed    5G    Controller
    Connect to Employee ssid and Check client can get ip    ${5G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.142 Connect to Changed Employee SSID 6G Controller
    [Tags]    Employee    Changed    6G    Controller
    Connect to Employee ssid and Check client can get ip    ${6G_Controller_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.143 Connect to Changed Employee SSID 2G Agent
    [Tags]    Employee    Changed    2G    Agent
    Connect to Employee ssid and Check client can get ip    ${2G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.144 Connect to Changed Employee SSID 5G Agent
    [Tags]    Employee    Changed    5G    Agent
    Connect to Employee ssid and Check client can get ip    ${5G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.145 Connect to Changed Employee SSID 6G Agent
    [Tags]    Employee    Changed    6G    Agent
    Connect to Employee ssid and Check client can get ip    ${6G_Agent_Employee_SSID}
    Verify Client cannot open GUI
    Verify Client and access to Internet
...
No.146 Change the Agent to Wireless connection
    Disable the switch port connected to the agent
    sleep    300
    Enable Client wired Network
    Check agent is wireless onboarding
...
###============================================================================
### 1) WiFi Zone/SSID 管理
### 2) Wizard/初始設定/Reset/Reboot
### 3) GUI Login/FW 查詢
### 4) Switch Port/Telnet 控制
### 5) Client 介面與 IP 操作
### 6) WPA Supplicant 設定與連線
### 7) 連線後驗證/打流量
### 8) Mesh/Agent 狀態/Topology
### 9) SSH 取得 SSID/BSSID/Password
### 10) Captive Portal 認證
### 11) WiFi 設定（Channel/MLO/Common）
### 12) 掃描/可見性驗證
### 13) SSH/GUI 啟用 SSH
### 14) Mesh 功能開關
### 15) Client MAC 操作
...
Scan SSID on client 
    ...
    Should Contain    ${result}   	${6G_Agent_SSIㄋD}    
